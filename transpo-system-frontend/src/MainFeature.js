...
const processRoute = useCallback(async (route) => {
    ...
    if (!passesJeepneyRoute) {
        return setTransportMode('⚠️ This route is not passable by public jeepneys. Use tricycle or private vehicle for the whole trip.');
    }
    ...
    let transferSteps = [];
    let walkabilityInfo = ''; // New variable for walkability summary
    ...
    const aiSuggestion = await generateInstructionGroq({
        ...
        walkability: walkabilityInfo, // <-- NEW CONTEXT
        routeSummary: routeSummary,   // <-- NEW CONTEXT
    });

    setIsAiProcessing(false);
    setTransportMode(aiSuggestion.replace(/\n/g, '<br/>'));
}, [discountType]); // Added processRoute dependencies for completeness
...
async function generateInstructionGroq(context) {
  ...
  const prompt = `
    You are a friendly and expert Public Transit Guide for the local **Norzagaray-Santa Maria, Bulacan Jeepney route**.
    ...
    ### Geographical Context (FOR YOUR REFERENCE):
        - The main route is generally along: **${context.routeSummary}**.
    - The Jeepney base fare is ₱12 for the first 4km, and ₱2 per succeeding kilometer.
    - All transfer steps (non-jeepney) should be assumed to require a tricycle.

    ### Required Travel Steps (MUST BE CONVERTED TO NUMBERED LIST):
    ${stepsBlock}

    ### Important Fare Information:
    - Total Distance: ${context.distance} km
    - Fare Calculation: Base Fare ₱${context.baseFare}. ${discountText}

    Provide the numbered list of directions first, followed by the "Friendly Tip" at the end. Use short, action-oriented sentences and make sure to remove asterisks.
    ${finalSummary}
  `;
  ...
  }, [selectedRouteIndex, directionsResult, discountType, processRoute]); // Only trigger when route index changes, not on initial load
            ...
            <div className="fare-content" id='intro'>
                ...
                <div className="input-group">
                    ...
                    <div className="discount-section">
                        ...
                        <select onChange={(e) => setDiscountType(e.target.value)} id='discount'>
